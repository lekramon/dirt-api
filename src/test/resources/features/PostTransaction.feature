...
Funcionalidade:
  ...
  Cenário: Registrar uma nova transação com sucesso
    Dado que a minha requisição seja feita com os seguintes parâmetros
    ...
    E a transação deve ser registrada na base dados
  ...
  Cenário: Registrar uma transação com falha
    Dado que a minha requisição seja feita com os seguintes parâmetros
      | ip             | amount | tax  | accountId | description         | captureMethodId | captureMethodType | transactionType | operation | otherAccountNumber | otherAccountAgency | otherAccountBankCode |
